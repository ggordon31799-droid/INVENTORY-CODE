 ...
 *   2. For each line: call inventoryService.applyDelta({ eventType: MANUAL_OUTBOUND })
 ...
 *   2. For each adjustment: call inventoryService.applyDelta({ eventType: CYCLE_COUNT })
 *   All within a single transaction.
 *
 * On confirmAdjustment():
 *   1. Create adjustment record
 *   2. Call inventoryService.applyDelta({ eventType: ADJUSTMENT })
 *   All within a single transaction.
 */
